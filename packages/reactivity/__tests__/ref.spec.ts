...
import { ref, effect, reactive, isReactive } from '../src/index'
import {
  isRef,
  Ref,
  unref,
...
describe('reactivity/ref', () => {
  ...
  it('ref wrapped in reactive should not track internal _value access', () => {
    ...
    // Reflect.set 执行一次  reactive的set执行一次
    b.value = 5
  ...
  it.skip('shallowRef', () => {
    const sref1 = shallowRef({ a: 1 })
    expect(isReactive(sref1.value)).toBe(false)

    let dummy4
    effect(() => {
      dummy4 = sref1.value.a
    })
    expect(dummy4).toBe(1)

    sref1.value = { a: 2 }
    expect(isReactive(sref1.value)).toBe(false)

    expect(dummy4).toBe(2)
  })
  ...
  test.skip('ref should preserve value shallow/readonly-ness', () => {
    const original = {}
