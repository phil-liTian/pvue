...
describe('reactivity/effectScope', () => {
  ...
  it('嵌套结构 stop的effectScope 也应该在parentScopes中移除', () => {
  ...
  it.skip('test with higher level APIs', async () => {
    const r = ref(1)
  ...
  it.skip('should pause/resume EffectScope', async () => {
    const counter = reactive({ num: 0 })
    const fnSpy = vitest.fn(() => counter.num)
  ...
  test.skip('removing a watcher while stopping its effectScope', async () => {
    const count = ref(0)
    const scope = effectScope()
    let watcherCalls = 0
    let cleanupCalls = 0
