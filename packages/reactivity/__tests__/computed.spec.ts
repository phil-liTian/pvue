 ...
 */
import { effect, ref, shallowRef } from '../src/index'
import {
  computed,
  ComputedRef,
  ComputedRefImpl,
...
describe('reactivity/computed', () => {
  ...
  it('should trigger effect', () => {
    ...
    // value.foo发生变化 会触发依赖更新 但是effect收集的依赖是cValue, 那么怎么重新执行呢？？？
    value.foo = 1
  ...
  it.skip('debug: onTrigger (reactive)', () => {
    let events: DebuggerEvent[] = []
  ...
  it('依赖多个属性时 添加到nextDep中', () => {
    ...
    e.value
    expect(e.deps!.nextDep!.dep).toBe(c.dep)
  ...
  it('get之后,computed的flags应该不是DIRTY的', () => {
    const v = ref(1)
  ...
  it('should not trigger effect scheduler by recursive computed effect', async () => {
    ...
    v.value += ' World'
    await nextTick()
    // console.log('serializeInner(root)', serializeInner(root))
  ...
  test('should not trigger if value did not change', () => {
    ...
    src.value = 2 // 如果computed的值没有发生变化 则effect应该不会再执行isDirty
  ...
  test.skip('chained computed avoid re-compute', () => {
    const effectSpy = vi.fn()
    const c1Spy = vi.fn()
    const c2Spy = vi.fn()
  ...
  it.skip('computed should force track in untracked zone', () => {
    const n = ref(0)
    const spy1 = vi.fn()
    const spy2 = vi.fn()
