...
import { useSlots } from '../src/apiSetupHelper'
import { ref } from '@pvue/reactivity'
...
describe('component: slots', () => {
  ...
  test('updateSlots: instance.slots should be updated correctly (when slotType is null)', async () => {
    ...
    const Comp = {
      setup() {
        return () => {
          return h(Child, { n: flag1.value }, flag1.value ? oldSlots : newSlots)
        }
      },
    }
    render(h(Comp), nodeOps.createElement('div'))
  ...
  test.skip('should respect $stable flag with a value of true', async () => {
    const flag1 = ref(1)
    const flag2 = ref(2)
    const spy = vi.fn()
  ...
  test.skip('should respect $stable flag with a value of false', async () => {
    const flag1 = ref(1)
    const flag2 = ref(2)
    const spy = vi.fn()
